...
def lose_game(update, play):
    ...
    info_player(update, "Lost a game ")
...
def set_lan(update, play):
    ...
    if play.set_lan and is_word:
        ...
        logger.info("Changed the language %s %s", update.message.from_user.first_name,
                    update.message.from_user.last_name)
        info_player(update, "Changed the language ")

    elif not is_word and not update.message.text == "/language" and \
            not update.message.text == "/language@WordGuesserbot":
...
def score(update, play):
    ...
    if update.message.text != "/reset" and update.message.text != "/reset@WordGuesserbot":
        ...
        info_player(update, "Asked for score ")
    else:
        del fil[str(play.id)]
        ...
        info_player(update, "Reseted score ")
...
def difficulty(update, play):
    ...
    info_player(update, "Changed difficulty ")
...
def definition(update, play):
    ...
    info_player(update, "Asked for definition ")
    ...
    if play.game_f_word != "":
        ...
        if play.set_trans and not (update.message.reply_text == play.language[27]
            ...
            if word_trans != "":
                ...
                update.message.reply_text("Sorry, try again! -> " + play.language[27],
                                          reply_markup=ReplyKeyboardRemove())
            play.set_trans = False
            logger.info("Asked for translate %s %s", update.message.from_user.first_name,
                        update.message.from_user.last_name)
            info_player(update, "Asked for translate ")
        else:
            i = 0
...
def add_player(update, list_id, list_player):
    try:
        if list_id.count(update.message.chat.id) == 0:
            ...
            info_player(update, "Player added ")
    ...
    if temp > 0:
        ...
        if update.message.from_user.last_name is not None:
            update.message.reply_text(
                msg + update.message.from_user.first_name + " " + update.message.from_user.last_name)
            ...
            try:
                if update.message.chat.id == list_player[i].id:
                    ...
                    if ((update.message.text == list_player[i].language[26]
                        ...
                        t = threading.Thread(target=definition, args=(update, list_player[i]))
                        t.start()
                    ...
                    if ((update.message.text == list_player[i].language[27]
                         ...
                         or list_player[i].set_trans)
                            and not (list_player[i].isplaying
                                     or list_player[i].set_lan
                                     or list_player[i].set_dif)):
                        t = threading.Thread(target=translate, args=(update, list_player[i]))
                        t.start()
                    # /about
                    ...
                    mode(update, list_player[i], bot, list_player)
